  ...
  return (
    <div className="p-6 max-w-2xl">
      ...
      {loading ? (
        ...
        <p className="text-ink-muted text-sm">No hay clubes todavía.</p>
      ) : (
        <div className="space-y-3">
          {clubs.map((club) => (
            <div key={club.id} className="bg-surface rounded-xl overflow-hidden">
              <button
                onClick={() => toggleClub(club.id)}
                className="w-full flex items-center justify-between px-4 py-3 text-left hover:bg-surface-hover transition-colors duration-150"
              >
                <div className="flex items-center gap-3">
                  <span className="text-ink font-medium">{club.name}</span>
                  <span className="text-xs text-ink-muted">{club.slug}</span>
                  {!club.is_active && (
                    <span className="text-xs bg-red-900 text-red-700 px-2 py-0.5 rounded-full">
                      inactivo
                    </span>
                  )}
                </div>
              ...
              {expandedClub === club.id && (
                <div className="border-t border-line px-4 py-3">
                  <p className="text-xs text-ink-muted uppercase tracking-wide mb-3">Usuarios</p>

                  {usersLoading === club.id ? (
                    <p className="text-ink-muted text-sm mb-3">Cargando...</p>
                  ) : (usersMap[club.id] ?? []).length === 0 ? (
                    <p className="text-ink-muted text-sm mb-3">Sin usuarios.</p>
                  ) : (
                      ...
                      {(usersMap[club.id] ?? []).map((u) => (
                        <li key={u.id} className="flex items-center justify-between">
                          <div>
                            ...
                            <span className="text-xs text-ink-muted ml-2">{u.email}</span>
                          </div>
                    ...
                    <form onSubmit={(e) => handleCreateUser(e, club.id)} className="space-y-2 mt-2">
                      <div className="grid grid-cols-2 gap-2">
                        <input
                          ...
                          className="col-span-2 bg-surface-strong text-ink text-sm rounded-lg px-3 py-2 placeholder-ink-faint outline-none focus:ring-1 focus:ring-brand-ring"
                        />
                        <input
                          required
                          type="email"
                          ...
                          className="bg-surface-strong text-ink text-sm rounded-lg px-3 py-2 placeholder-ink-faint outline-none focus:ring-1 focus:ring-brand-ring"
                        />
                        <input
                          required
                          type="password"
                        ...
                        <select
                          ...
                          className="col-span-2 bg-surface-strong text-ink text-sm rounded-lg px-3 py-2 outline-none focus:ring-1 focus:ring-brand-ring"
                        >
                      ...
                      {userError && (
                        <p className="text-red-600 text-xs">{userError}</p>
                      )}
      ...
      {showClubModal && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-4 animate-overlay">
          <div className="bg-surface rounded-2xl w-full max-w-md p-6 animate-modal">
            <h2 className="text-ink font-bold text-lg mb-4">Nuevo club</h2>
            <form onSubmit={handleCreateClub} className="space-y-3">
              ...
              />
              <p className="text-xs text-ink-muted pt-1">Admin del club</p>
              <input
                required
                placeholder="Nombre completo"
              ...
              <input
                ...
                className="w-full bg-surface-strong text-ink text-sm rounded-lg px-3 py-2.5 placeholder-ink-faint outline-none focus:ring-1 focus:ring-brand-ring"
              />
              {clubError && <p className="text-red-600 text-xs">{clubError}</p>}
